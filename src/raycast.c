...
void	raycast(t_data *data)
{
	...
	for (x = 0; x < NUM_RAYS; x++)
	{
		...
		side_dist_x = (dir_x < 0) ? (data->player.x - map_x)
			* delta_dist_x : (map_x + 1.0 - data->player.x) * delta_dist_x;
		side_dist_y = (dir_y < 0) ? (data->player.y - map_y)
			* delta_dist_y : (map_y + 1.0 - data->player.y) * delta_dist_y;
		...
		while (!hit)
		{
			...
			if (map_y >= 0 && map_y < data->map_height && map_x >= 0
				&& map_x < data->map_width && data->mapa[map_y][map_x] == '1')
			{
				hit = 1;
				if (side == 0)
		...
		int wall_height = (int)(NOVA_ALTURA / (perp_wall_dist
					* cos(data->player.angle - ray_angle)));
